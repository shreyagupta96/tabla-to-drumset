"""
Training script for Tabla LSTM model WITHOUT embedding regularization
...
def train_epoch(model, dataloader, criterion_note, criterion_dur, optimizer, device):
    """Train for one epoch WITHOUT embedding similarity loss"""
    model.train()
    total_loss = 0
    total_note_loss = 0
    total_dur_loss = 0
    ...
    for batch in dataloader:
        ...
        # Calculate prediction losses ONLY (no embedding regularization)
        loss_note = criterion_note(note_logits, y_notes)
        loss_dur = criterion_dur(dur_logits, y_durs)

        # Combined loss (prediction only)
    ...
    return avg_loss, avg_note_loss, avg_dur_loss, note_acc, dur_acc
...
def train_model(data_file, num_epochs=100, batch_size=32, learning_rate=0.001,
                val_split=0.2, save_path='tabla_lstm_model_no_embeddings.pth'):
    """
    Main training function WITHOUT embedding regularization
    ...
    # Load data
    print("="*80)
    print("TRAINING TABLA LSTM MODEL (NO EMBEDDING REGULARIZATION)")
    print("="*80)
    ...
    # Train/val split (use same random seed for fair comparison)
    np.random.seed(42)
    ...
    print(f"⚠️  NO embedding regularization (baseline model)")
    print("-"*80)
    ...
    for epoch in range(num_epochs):
        ...
        # Train WITHOUT embedding similarity loss
        train_loss, train_note_loss, train_dur_loss, train_note_acc, train_dur_acc = train_epoch(
            model, train_loader, criterion_note, criterion_dur, optimizer, device
        )
...
if __name__ == "__main__":
    ...
    MODEL_PATH = "/Users/shreyagupta/Desktop/AI_Research_Data/tabla-to-drumset_SG/tabla_lstm_model_no_embeddings.pth"
